...
const kuberScore = calculateKuberScore(mockUser);
const scoreCategory = getScoreCategory(kuberScore);
const allocation = calculateSalaryAllocation(mockUser);
const emergencyMonths = Math.round((mockUser.currentSavings / mockUser.monthlyExpenses) * 10) / 10;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-display font-bold text-foreground">
          Welcome back, <span className="gold-text">Arjun</span>
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-4 text-center max-w-[200px]">
            Your savings rate of {Math.round(((mockUser.monthlySalary - mockUser.monthlyExpenses) / mockUser.monthlySalary) * 100)}% and {emergencyMonths} months emergency fund contribute to your score.
          </p>
        </motion.div>

        <div className="lg:col-span-2 grid grid-cols-2 gap-4">
          <MetricCard icon={IndianRupee} label="Monthly Salary" value={`₹${mockUser.monthlySalary.toLocaleString()}`} variant="gold" />
          <MetricCard icon={Wallet} label="Expenses" value={`₹${mockUser.monthlyExpenses.toLocaleString()}`} sub={`${Math.round((mockUser.monthlyExpenses / mockUser.monthlySalary) * 100)}% of salary`} variant="warning" />
          <MetricCard icon={PiggyBank} label="Savings" value={`₹${mockUser.currentSavings.toLocaleString()}`} sub={`${emergencyMonths} months buffer`} variant="success" />
          <MetricCard icon={Shield} label="EMIs" value={`₹${mockUser.existingEMIs.toLocaleString()}`} sub={`${Math.round((mockUser.existingEMIs / mockUser.monthlySalary) * 100)}% of salary`} variant="destructive" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="glass-card rounded-2xl p-6 border border-border">
          ...
          <div className="space-y-4">
            {[
              { label: 'Essentials', amount: allocation.essentials, pct: allocation.essentials / mockUser.monthlySalary, color: 'bg-primary' },
              { label: 'Investments', amount: allocation.investments, pct: allocation.investments / mockUser.monthlySalary, color: 'bg-success' },
              { label: 'Savings', amount: allocation.savings, pct: allocation.savings / mockUser.monthlySalary, color: 'bg-chart-3' },
              { label: 'Lifestyle', amount: allocation.lifestyle, pct: allocation.lifestyle / mockUser.monthlySalary, color: 'bg-chart-4' },
            ].map((item) => (
              <div key={item.label}>
